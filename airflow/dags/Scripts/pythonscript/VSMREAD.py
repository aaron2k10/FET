...
for i in data['transf']:
    ...
    name = i['name']
    if(i['type']!="ch"):
        dtype1[name]=Numeric(bytes)
    else:
        dtype1[name]=VARCHAR(bytes)
with db.connect() as conn:
    df.to_sql('READACCT', con=conn, if_exists='replace', index=False,dtype=dtype1)
readacct=pd.read_sql_table('READACCT',con=db)
# Download the vsamdataset from PostgreSQL into a txt file using the same copybook

with open(file_path, "w") as file:
    for index, row in readacct.iterrows():
        line = ""
